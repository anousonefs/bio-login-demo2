...
func initCrypto() {
	if _, err := os.Stat(config.dotKeys); err != nil {
		...
		if err != nil {
			log.Printf("ERROR: fail to create keys dir, %v", err.Error())
			os.Exit(1)
		}
	}
...
func getIdRsa() *rsa.PrivateKey {
	keyData, err := os.ReadFile(config.idRsa)
	if err != nil {
...
func getIdRsaPub() *rsa.PublicKey {
	keyData, err := os.ReadFile(config.idRsaPub)
	if err != nil {
...
func getIdRsaPubFromStr(keyStr string) *rsa.PublicKey {
	...
	if rest, err := asn1.Unmarshal(data, &pubKey); err != nil {
		log.Printf("INFO: not ios key %v", keyStr)
	} else if len(rest) != 0 {
